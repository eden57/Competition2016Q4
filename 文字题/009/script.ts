...
function getValue():void{
    ...
    let strs = processString([this.test],sign,false);
    console.log(strs);
    process2(strs);
 ...
 * @returns {any}
 */
function processString(str:string[],sign:string[],end:boolean):string[]{
    if(end){
        return str;
    }
    let result = [];
...
function process2(test:string[]):void{
    for(let str of test){
        for(let i=0;i<str.length;i++){
            i = process3(str,i);
        }
...
function process3(str:string,index:number):number{
    let theNum;
    //如果是数字
    if(isNum(str.charAt(index))&&index<str.length-1){
        ...
        if(count===liangci.length){
            ...
            if(!isNum(str.charAt(index))&&index<str.length){
                //验证是不是名词
                let loc = porcess4(str.slice(index-1,str.length));
                if(loc[0]>=0){
                    //将结果存储
