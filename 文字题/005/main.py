...
def main():
    print("对第一话全文捕获关键词大约需要2至3分钟，请倒杯茶等待")
    ...
    for item in result:
        ...
        get_image(item['m'], get_picture_url(item['n']), index)
        print('导出了第%s张图片' % index)
...
def progress(text):
    ...
    # 原本试图删除数组中的元素 发现index会乱 以后改用filter 切记
...
def get_image(m, url, index):
    # a = urllib.parse.splittype(url)
    ...
    data = res.read()
    for item in range(m):
        f = open('%s_%s.jpg' % (index, item + 1), 'wb')
        f.write(data)
        im = Image.open('%s_%s.jpg' % (index, item + 1))
        w, h = im.size
        im.thumbnail((w / h * 50, 50))
        # 把缩放后的图像用jpeg格式保存:
        im.save('%s_%s.jpg' % (index, item + 1), 'jpeg')
